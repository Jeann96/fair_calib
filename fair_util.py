...
def load_data(nconfigs):
    ...
    solar_forcing = np.zeros(271)
    volcanic_forcing = np.zeros(271)
    ...
    solar_forcing = df_solar["erf"].loc[1750:2020].values
    ...
    da = da_emissions.loc[dict(config="unspecified", scenario='ssp245')][:270, ...]
...
def runFaIR(solar_forcing, volcanic_forcing, emissions, df_configs):
    '''
...
# TODO: Replace opening configs with MCMC sampling
...
if len(df_configs.shape)<2:
    nconfigs=1
    df_configs=df_configs.to_frame().transpose()
else:
    nconfigs=df_configs.shape[0]
        
# TODO: Move this outside this module and import load_data and runFaIR. One call
# for load_data is enough before starting the MCMC. Only df_configs is updated
# for each call
solar_forcing, volcanic_forcing, emissions = load_data(nconfigs)
f=runFaIR(solar_forcing, volcanic_forcing, emissions, df_configs)
